  ...
  const uploads = await Promise.all(
    formData.images.map(async (file, idx) => {
      try {
        ...
        console.log("Uploaded file", idx, "URL:", url);
        return url;
      } catch (err) {
        console.error("Error uploading file", idx, file?.name, err);
        throw err; // stop the whole process on error
      }
    })
  );
  imageUrls = uploads;
  ...
  return (
    <div style={{ maxWidth: 800, margin: "2rem auto" }}>
      <Form
        ...
        widgets={{ imageUpload: ImageUploadWidget }}
        formData={formData}
        validator={validator}
